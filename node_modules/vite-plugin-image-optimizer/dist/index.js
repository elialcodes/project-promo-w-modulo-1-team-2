...
function ViteImageOptimizer(optionsParam = {}) {
  ...
  const applySharp = async (filePath, buffer) => {
    ...
    const extName = pathe.extname(filePath).replace(".", "").toLowerCase();
  ...
  const processFile = async (filePath, buffer) => {
    try {
      let newBuffer;
      let isCached;
      const cachedFilePath = pathe.join(options.cacheLocation, filePath);
      ...
      if (options.cache === true && !isCached) {
        if (!fs.existsSync(pathe.dirname(cachedFilePath))) {
          await fsp.mkdir(pathe.dirname(cachedFilePath), { recursive: true });
        }
  ...
  return {
    ...
    async closeBundle() {
      if (publicDir && options.includePublic) {
        ...
        const files = getFilesToProcess(allFiles, (path) => filename(path) + pathe.extname(path));
        if (files.length > 0) {
          await ensureCacheDirectoryExists();
